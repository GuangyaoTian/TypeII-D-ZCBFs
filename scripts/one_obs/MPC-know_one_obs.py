...
### constraints
g = [] 
### the Information of obstacle have knowen before:
x_obs_list=np.array([1.5])
y_obs_list=np.array([2])

### In this way,we can make the constrains before the while 
for i in range(N+1):
    for m in range(1):
...
while not flag: 
    if current_target_index<1 :
        ...
        for m in range(1):
            distlist.append(ca.sqrt((x_real-x_obs_list[m])**2+(y_real-y_obs_list[m])**2)-0.1) 
