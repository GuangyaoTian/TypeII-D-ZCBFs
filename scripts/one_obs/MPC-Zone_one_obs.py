...
def scan_callback(msg):
    ...
    for i, distance in enumerate(msg.ranges):
        ### in gazebo diatance may be equal to inf
    ...
    for label in unique_labels:
        ...
        if len(points) > 0:
            ...
            ellipse_array = [center[0], center[1], a , b  , angle_radians]  # 旋转角度设为0
...
while not flag: 
    if current_target_index<1 :
        ...
        ###add constrans to avoid the obstacle

        if(len(ellipse_arrays_np)>0):
            distlist=[]
            for m in range(len(ellipse_arrays_np)):
                obs=ellipse_arrays_np[m]
                dd_dis=np.sqrt((x_real-obs[0])**2+(y_real-obs[1])**2)-obs[2]
                dd_dis2=np.sqrt((x_real-obs[0])**2+(y_real-obs[1])**2)-obs[2]
                distlist.append(dd_dis2)
                if dd_dis < 0.6:
            ...
            if obsnumis==1:         
                nlp_prob = {'f': obj, 'x': ca.reshape(U, -1, 1), 'p':P, 'g':ca.vertcat(*g)}
                opts_setting = {'ipopt.max_iter':100, 'ipopt.print_level':0, 'print_time':0, 'ipopt.acceptable_tol':1e-8, 'ipopt.acceptable_obj_change_tol':1e-6}
                ### make solver again (add obstacle)
                solver = ca.nlpsol('solver', 'ipopt', nlp_prob, opts_setting)
                ifobs=1
            elif ifobs==1:
                g=[]
