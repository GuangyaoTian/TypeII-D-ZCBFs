...
### to know the turtlebot stop or continue
position_history=[]
is_stopped = False
stoptime=0
mpctime=0
...
xs = np.array([2.5,3,0]).reshape(-1, 1)
...
### target point
target_points = [
        np.array([2.5, 2.5, 0]).reshape(-1, 1)
    ]
...
while not flag: 
    if current_target_index<1 :
        ...
        if not is_stopped:### still use TypeII D ZCBFs
            if num_ellipses2 > 0:
                for m in range(len(ellipse_arrays_np)):
                    ...
                    if CBF<=0.3:
                        ...
                        c_cbf=np.zeros(2)
                        b_cbf=np.array([CBF_Condition,v_max,v_max,omega_max,omega_max])
                        ...
                        A = np.array([[e1, e2],[1,0],[-1,0],[0,1],[0,-1]])
                        u1 = cp.Variable()
                        u2 = cp.Variable()
                        objective = cp.Minimize(0.5 * cp.quad_form(cp.vstack([u1-0.3, u2]), Q_cbf) + c_cbf @ cp.vstack([u1, u2]))
        ...
        position_history.append((x_real, y_real,theta_real))
        if len(position_history) == 8:
            stopped = True
            for i in range(1, len(position_history)):
                dx = abs(position_history[i][0] - position_history[i-1][0])
                dy = abs(position_history[i][1] - position_history[i-1][1])
                dtheta= abs(position_history[i][2] - position_history[i-1][2])
                distoobs= ((x_real+l*np.cos(theta_real)-1.5)**2+(y_real-0.5+l*np.sin(theta_real))**2)**0.5-obs_diam
                if dx >= 0.002 or dy >= 0.002 or dtheta>= 0.005 or distoobs>0.5:
                    stopped = False
                    break
            if stopped:
                is_stopped = True
                ### the Turtlebot is stopped because of the u_real[0] and u_real[1] is too small
                ### change MPC add constraints to avoid the obstacle and go to the goal point
                for i in range(N+1):
                    g.append(ca.sqrt((X[0, i]-1.5)**2+(X[1, i]-0.5)**2)) 
                for _ in range(N+1):
                    lbg.append(0.4)
                    ubg.append(np.inf)
                nlp_prob = {'f': obj, 'x': ca.reshape(U, -1, 1), 'p':P, 'g':ca.vertcat(*g)}
                ...
                if is_stopped:
                    ...
                    obs_x=1.5
                    obs_y=0.5
                    distance= ((x_real+l*np.cos(theta_real)-obs_x)**2+(y_real-obs_y+l*np.sin(theta_real))**2)**0.5-obs_diam
                    ### time to go back to TypeII ZCBFs
                    if distance > 0.2:
                        lbg = []
                        g = []
                        ubg = []
