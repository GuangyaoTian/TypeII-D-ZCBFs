...
### scan_callback is useless in this code
def scan_callback(msg):
    ...
    if points_array.size == 0: ### no obstacle from Lidar
        num_ellipses = 0  
        ellipse_arrays = []  
        return  
...
x_obs_list=np.array([0.75,1.5,2.25,0.75,2.25])
y_obs_list=np.array([0.75,1.5,2.25,2.25,0.75])

### the informations of obstacles are knowen before while
for i in range(N+1):
    for m in range(5):
        g.append(ca.sqrt((X[0, i]-x_obs_list[m])**2+(X[1, i]-y_obs_list[m])**2)) 
...
while not flag: 
    if current_target_index < 1 :
        ...
        pub.publish(speed)
        if np.linalg.norm(x_idk-xs)<0.5:
