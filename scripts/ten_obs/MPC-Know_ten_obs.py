...
### in this code, because the informations of obstacles are knowen before, the scan_call_back is useless
...
### the informations of obstacles are knowen before code
x_obs_list=np.array([1.5,0.75,0.75,2.25,2.25,-0.75,1.5,0.75,-0.75,2.95])
y_obs_list=np.array([1.5,0.75,2.25,0.75,2.25,-0.75,0.05,-1,2.25,1.5])
...
for _ in range(N+1): ### avoid ten obstacles at the same time
    lbg.append(0.4)
    ubg.append(np.inf)
    lbg.append(0.4)
    ubg.append(np.inf)
...
### target points (in this code There are three target points)
target_points = [
...
while not flag: 
    if current_target_index<3 :
        ...
        for m in range(10):
            distlist.append(ca.sqrt((x_real-x_obs_list[m])**2+(y_real-y_obs_list[m])**2)) # should be smaller als 0.0
