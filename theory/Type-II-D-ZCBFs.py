...
if __name__ == '__main__':
    ...
    t0 = 0.0
    x0 = np.array([0.0, 0.0, np.pi/2]).reshape(-1, 1)
    xs = np.array([0, 2, 0]).reshape(-1, 1) 
    ...
    y_obs_list=np.array([1,2.0,1,0.1])
    r_obs_list=np.array([0.2,0.2,0.2,0.2])
    safe_dist=0.2
    ...
    while(len(target_states) > 0):
        ...
        while np.linalg.norm(x0-xs) > 1e-1 and mpciter < sim_time/T:
            ...
            ###CBF
            while m<4:
                distance= ((x0[0]+l*np.cos(x0[2])-x_obs_list[m])**2+(x0[1]-y_obs_list[m]+l*np.sin(x0[2]))**2)**0.5-r_obs_list[m]-safe_dist
                if distance<=0.3:
                    CBF_Condition=distance
                    Q_cbf=np.array([[10000, 0], [0, 3]])
                    CBF_Condition=CBF_Condition.full()[0][0]
                    u1 = cp.Variable()
                    u2 = cp.Variable()

                    e1=-0.5*((x0[0]+l*np.cos(x0[2])-x_obs_list[m])**2+(x0[1]-y_obs_list[m]+l*np.sin(x0[2]))**2)**(-0.5)*2*((x0[0]+l*np.cos(x0[2])-x_obs_list[m])*np.cos(x0[2])+(x0[1]-y_obs_list[m]+l*np.sin(x0[2]))*np.sin(x0[2]))
                    e2=-0.5*((x0[0]+l*np.cos(x0[2])-x_obs_list[m])**2+(x0[1]-y_obs_list[m]+l*np.sin(x0[2]))**2)**(-0.5)*2*((x0[0]+l*np.cos(x0[2])-x_obs_list[m])*l*(-1)*np.sin(x0[2])+(x0[1]-y_obs_list[m]+l*np.sin(x0[2]))*l*np.cos(x0[2]))
                    e1=e1.full()[0][0]
                    e2=e2.full()[0][0]
                    ...
                    objective = cp.Minimize(0.5 * cp.quad_form(cp.vstack([u1-v_max, u2]), Q_cbf) )
